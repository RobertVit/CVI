...
* Generated by SAP_LOCAL_DOWNPORT_ASSISTANT Version 7.22 on 09.04.2019 with option ALL

* The individual coding starts at 'FORM update_01.' ( line 5.800 )
...
*       GOTSTATE      = GOTSTATE
  ...
  METHOD generate_maint_view.   "call SE54, leave screens untouched
    ...
    "Check situation of note 2328941 - SE54 | SV349 | You have no change authorization
    SELECT COUNT( * ) FROM smodilog WHERE obj_type EQ 'FUGR'
                                      AND obj_name EQ i_func_group
                                      AND active   EQ 'U'.
    IF sy-subrc EQ 0.
      MESSAGE w349(sv) INTO sy-lisel.                       "#EC *
      CALL METHOD lcl_wb=>log_message.
      MESSAGE e666(01) WITH 'Table/View' i_objectname 'must be adjusted in SPAU first.' 'Please follow note 2328941' INTO sy-lisel. "#EC *
      CALL METHOD lcl_wb=>log_message.
      RETURN.
    ENDIF.
      ...
      WHEN 'V' OR 'S'.   "view or table
        ...
        CALL FUNCTION 'RS_ACCESS_PERMISSION'
          EXPORTING
            ...
            object           = i_func_group
            object_class     = 'FUGR'
            dequeue_synchron = 'X'            ">=SAPBASIS720
          EXCEPTIONS
            OTHERS           = 1.
        IF sy-subrc NE 0.
          sy-msgty = 'E'.
...
*          "-------------------------------------------
      ...
      WHEN 'C'.       "view cluster
        ...
        bdc_screen  'SAPLSPO1'   '0100'.    "Should the subobject maintenance screens be modified?
        bdc_field   'BDC_OKCODE' '=NO'.
        bdc_screen  'SAPL0SVM'   '0101'.
    ...
    IF testrun IS INITIAL.
      ...
      "create texts in logon language if neccessary.
      SET LOCALE LANGUAGE space.
      IF sy-langu NE i_masterlang.  "SE54 requires to habe text in logon language
        CASE i_objecttype.
          WHEN 'C'.
            DATA: lt_vcldirt TYPE TABLE OF vcldirt, ls_vcldirt TYPE vcldirt.
            SELECT * FROM vcldirt INTO TABLE lt_vcldirt WHERE vclname EQ i_objectname AND spras EQ i_masterlang.
            ls_vcldirt-spras = sy-langu.
            MODIFY lt_vcldirt FROM ls_vcldirt TRANSPORTING spras WHERE table_line IS NOT INITIAL.
            INSERT vcldirt FROM TABLE lt_vcldirt ACCEPTING DUPLICATE KEYS.

            DATA: lt_vclstruct TYPE TABLE OF vclstruct, ls_vclstruct TYPE vclstruct.
            SELECT * FROM vclstruct INTO TABLE lt_vclstruct WHERE vclname EQ i_objectname AND spras EQ i_masterlang.
            ls_vclstruct-spras = sy-langu.
            MODIFY lt_vclstruct FROM ls_vclstruct TRANSPORTING spras WHERE table_line IS NOT INITIAL.
            INSERT vclstruct FROM TABLE lt_vclstruct ACCEPTING DUPLICATE KEYS.
        ENDCASE.
      ENDIF.

      "prepare call of function CLMINT_GET_MODIFICATION_STATE
      DATA: lt_smodilog_adjust TYPE TABLE OF smodilog,      ">20180411
            l_func_master TYPE progname.
      l_func_master = lcl_wb=>get_function_pool_master( i_func_group ).
      SELECT * FROM smodilog INTO TABLE lt_smodilog_adjust
        WHERE obj_type = 'FUGR'
          AND obj_name = i_func_group
          AND ( sub_type = 'REPS' AND sub_name EQ l_func_master OR sub_type EQ 'DYNP' )
          AND prot_only = ' '
          AND upgrade   = 'X'.
      IF lt_smodilog_adjust[] IS NOT INITIAL.
        CONCATENATE 'REPS' l_func_master INTO sy-lisel SEPARATED BY space.
        IF tse54im IS INITIAL.
          MESSAGE e526(eu) WITH sy-lisel INTO sy-lisel.     "#EC *
          CALL METHOD log_message.
          RETURN.
        ELSE.
          MESSAGE w526(eu) WITH sy-lisel INTO sy-lisel.     "#EC *
          CALL METHOD log_message.
          DELETE smodilog FROM TABLE lt_smodilog_adjust.
        ENDIF.
      ENDIF.                                                "<20180411
...
*    ls_opt-NOBIEND = 'X'. " Batchinputmodus nach Ende der BDC-Daten
      ...
      LOOP AT lt_message INTO ls_message.
        ...
        IF sy-msgid EQ 'SV'.
          ...
          ENDCASE.
        ELSEIF sy-msgid EQ '00' AND sy-msgno EQ 344. "no batch input data for screen = unexpected popup
          gv_errors_occured = 'X'. sy-msgty = 'W'. r_mode = 'E'.  "no success -> return mode 'Error'
        ENDIF.
        CALL METHOD log_message.
      ENDLOOP.

      IF lt_smodilog_adjust[] IS NOT INITIAL.               ">20180411
        INSERT smodilog FROM TABLE lt_smodilog_adjust.
      ENDIF.                                                "<20180411
...
*      "set some flag which cannot be set via batch input because this is done in SOBJ but not in SE54!
      UPDATE objh SET importable = i_compare_flag objtransp = i_transp_flag cursetting = i_cursetting "20180411
    ...
    IF testrun IS INITIAL.
      CASE i_object.
        ...
        WHEN 'TRAN'.
          ...
          DELETE FROM tstct   WHERE tcode EQ i_obj_name.
...
*                                                 OR ( decimals    NE 0 )       " works with = '500.12 ' (independent from sy-langu)
    ...
    CASE i_tabname.
      ...
      WHEN 'T511K' OR 'T511P' OR
           ...
           'DDDDLSRC' OR   "'DDLS' , usually delivered via SNOTE
...
*           ACTCLASS      = ' '
  ...
  METHOD create_function_group.
    ...
    DATA: l_text       LIKE i_text,
          ...
          l_master     TYPE progname,
          ls_fugrname  LIKE LINE OF gr_fugrname VALUE 'IEQ'.
    ...
    ls_fugrname-low = i_area.
    COLLECT ls_fugrname INTO gr_fugrname.
...
*       LIMU_KEY  =
      EXCEPTIONS
        ...
        OTHERS    = 2.
    IF sy-subrc EQ 0.
      "check if master program exists, too.
      CALL FUNCTION 'RPY_EXISTENCE_CHECK_PROG'
        EXPORTING
          name      = l_master
        EXCEPTIONS
...
*         WI_DELETE_TADIR_ENTRY = ' '
    ...
    "cleanup some tables if neccessary
    DELETE FROM tlibg WHERE area EQ i_area.
    DELETE FROM tlibt WHERE area EQ i_area AND spras EQ i_langu.
...
*       NEW_NAME               =
    ...
    "update TADIR master language and TLIBT if necessary
    UPDATE tadir SET masterlang = i_langu
               WHERE pgmid      EQ 'R3TR'
                 AND object     EQ 'FUGR'
                 AND obj_name   EQ i_area
                 AND masterlang NE i_langu.
    UPDATE tlibt SET spras      = i_langu
               WHERE spras      EQ sy-langu
                 AND area       EQ i_area
                 AND areat      EQ l_text.
    "check if master program has been created as expected (may fail if use changed group name in popup)
    CALL FUNCTION 'RPY_EXISTENCE_CHECK_PROG'
      EXPORTING
        name      = l_master
      EXCEPTIONS
    ...
    IF sy-subrc EQ 0.
      MESSAGE i666(01) WITH 'Update successful' INTO sy-lisel. "#EC *
    ELSE.
      MESSAGE e666(01) WITH 'Update failed' INTO sy-lisel.  "#EC *
    ENDIF.
    CALL METHOD log_message.
...
*      APPEND INITIAL LINE TO lt_textpool ASSIGNING <ls_textpool>.
  ...
  METHOD create_user_interface.
    ...
    IF sy-subrc NE 0.
      ...
      CALL METHOD log_message.
      DELETE gr_guistatus WHERE low EQ i_progname. "V7.21     "remove from activation worklist
      RETURN.
    ENDIF.
...
FORM update_01. " NROB CVI_PRECHK ( EN , source: E18 )

  CALL METHOD lcl_wb=>create_logical_object "#EC *
    EXPORTING
      i_object     = 'NROB'
      i_obj_name   = 'CVI_PRECHK'
  ...
  DATA: ls_tnros TYPE tnros, lt_tnros TYPE TABLE OF tnros.

  CLEAR ls_tnros.
  ls_tnros-object           = 'CVI_PRECHK'.                                                         "#EC NOTEXT
  ls_tnros-domlen           = 'CVI_PRECHK_RUNID'.                                                   "#EC NOTEXT
  ls_tnros-percentage       = '10.0 '.                                                              "#EC NOTEXT
  APPEND ls_tnros TO lt_tnros.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'NROB'
      i_tabname  = 'TNROS'
      it_entries = lt_tnros.

  DATA: ls_tnrost TYPE tnrost, lt_tnrost TYPE TABLE OF tnrost.

  CLEAR ls_tnrost.
  ls_tnrost-object       = 'CVI_PRECHK'.                                                            "#EC NOTEXT
  ls_tnrost-langu        = 'E'.                                                                     "#EC NOTEXT
  ls_tnrost-txt          = 'Number Range for Migration Precheck'.                                   "#EC NOTEXT
  ls_tnrost-txtshort     = 'CVI Precheck'.                                                          "#EC NOTEXT
  APPEND ls_tnrost TO lt_tnrost.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'NROB'
      i_tabname  = 'TNROST'
      it_entries = lt_tnrost.

ENDFORM.

FORM update_02. " FUGR CVI_PRECHK ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_function_group   "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_appl       = 'S'
      i_text       = 'CVI precheck background task'.

ENDFORM.

FORM update_03. " FUGR FG_PRECHK_SUP ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_function_group   "#EC *
    EXPORTING
      i_devclass   = 'CVI_MAPPING'
      i_area       = 'FG_PRECHK_SUP'
      i_langu      = 'E'
      i_appl       = 'S'
...
FORM update_04. " DOCU DECVI_PRECHK_ADDR ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_ADDR'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Address </>data is being considered for precheck.',                     "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_05. " DOCU DECVI_PRECHK_BANK ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_BANK'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Bank </>data is being considered for precheck.',                        "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_06. " DOCU DECVI_PRECHK_BGMODE ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_BGMODE'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that the report is in background processing mode',                               "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS By default, the background processing is selected. Uncheck if you want',                   "#EC NOTEXT
                    '   to run the report in foreground mode.',                                                    "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_07. " DOCU DECVI_PRECHK_EMAIL ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_EMAIL'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Email </>scenario is being considered for precheck.',                   "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS You can validate the Email main contact for the selected master data.',                    "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_08. " DOCU DECVI_PRECHK_INDUS ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_INDUS'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Industry </>scenario is being considered for',                          "#EC NOTEXT
                    '   precheck.',                                                                                "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
...
FORM update_09. " DOCU DECVI_PRECHK_NUMRANGE ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_NUMRANGE'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Number Range </>scenario is being considered for',                      "#EC NOTEXT
                    '   precheck',                                                                                 "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
...
FORM update_10. " DOCU DECVI_PRECHK_OBJTYPE ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_OBJTYPE'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates whether the master data belongs to customer or vendor.',                         "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_11. " DOCU DECVI_PRECHK_PCODE ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_PCODE'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Postal Code</> scenario is being considered for',                       "#EC NOTEXT
                    '   precheck.',                                                                                "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS You can validate the <LS>Postal Address </>of selected master data.',                      "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_12. " DOCU DECVI_PRECHK_RUNID ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_RUNID'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Denotes the unique identifier for each run.',                                              "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS You can click the run ID to view the result of each run.',                                 "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_13. " DOCU DECVI_PRECHK_RUN_DESC ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_RUN_DESC'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates the description for each run.',                                                  "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS You can enter a description for each run. This field is mandatory.',                       "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_14. " DOCU DECVI_PRECHK_SCENARIO ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_SCENARIO'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates the scenario that is being considered for migration precheck.',                  "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_15. " DOCU DECVI_PRECHK_SELECTALL ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_SELECTALL'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Denotes that all standard business checks are selected for master data',                   "#EC NOTEXT
                    '   consistency check.',                                                                       "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS By default, the <LS>Select All</> check box is checked. You can uncheck',                  "#EC NOTEXT
                    '   the business scenarios that you do not want to check for master data',                     "#EC NOTEXT
                    '   consistency.',                                                                             "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_16. " DOCU DECVI_PRECHK_STATUS ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_STATUS'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates whether the precheck is in process<(>,<)> or completed, or',                     "#EC NOTEXT
                    '   terminated due to a system error.',                                                        "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'B1 If the precheck is completed, double-click the <LS>Details </>icon to',                    "#EC NOTEXT
                    '   view error details',                                                                       "#EC NOTEXT
                    'B1 If the precheck is terminated, double-click the <LS>Details </>icon to',                   "#EC NOTEXT
                    '   view details',                                                                             "#EC NOTEXT
                    'B1 You can use <LS>Refresh </>button to get the latest status of a run',                      "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_17. " DOCU DECVI_PRECHK_SUP_IND ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_SUP_IND'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Controls activation of suppression checks before migrating to SAP',                        "#EC NOTEXT
                    '   S/4HANA',                                                                                  "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS Select the relevant check box to activate suppression check.',                             "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_18. " DOCU DECVI_PRECHK_SUP_TYPE ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_SUP_TYPE'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Denotes the functional checks that can be suppressed during conversion',                   "#EC NOTEXT
                    '   process.',                                                                                 "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS You can bypass the following field checks during conversion to SAP',                       "#EC NOTEXT
                    '   S/4HANA:',                                                                                 "#EC NOTEXT
                    'B1 Country',                                                                                  "#EC NOTEXT
                    'B1 Region',                                                                                   "#EC NOTEXT
                    'B1 Postal Code',                                                                              "#EC NOTEXT
                    'B1 Tax Jurisdiction',                                                                         "#EC NOTEXT
                    'B1 Email',                                                                                    "#EC NOTEXT
                    'B1 Bank',                                                                                     "#EC NOTEXT
                    'B1 Geo-code',                                                                                 "#EC NOTEXT
                    'B1 Mandatory fields',                                                                         "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
...
FORM update_19. " DOCU DECVI_PRECHK_TAXCODE ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_TAXCODE'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Tax Code </>scenario is being considered for',                          "#EC NOTEXT
                    '   precheck.',                                                                                "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS You can validate fields such as <LS>VAT ID</>, <LS>Tax Number 1</>,',                      "#EC NOTEXT
                    '   <LS>Tax Number 2</>, <LS>Tax Number 3 </>for the selected master data.',                   "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_20. " DOCU DECVI_PRECHK_TAX_J ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_TAX_J'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Tax Jurisdiction </>is being considered for precheck.',                 "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS You can validate <LS>Tax Type </>of the selected master data.',                            "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_21. " DOCU DECVI_PRECHK_TZONE ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_dtel_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_rollname  = 'CVI_PRECHK_TZONE'
      i_langu     = 'E'
      i_line      =:'U1 &DEFINITION&',                                                                             "#EC NOTEXT
                    'AS Indicates that <LS>Transportation Zone</> is being considered for',                        "#EC NOTEXT
                    '   precheck.',                                                                                "#EC NOTEXT
                    'U1 &USE&',                                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &DEPENDENCIES&',                                                                           "#EC NOTEXT
...
FORM update_22. " REPT CL_CVI_PRECHK_UI==============CP ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_program_text    "#EC *
    EXPORTING
      i_progname   = 'CL_CVI_PRECHK_UI==============CP'
      i_devclass   = 'CVI_MAPPING'
      i_masterlang = 'E'
      i_langu      = 'E':
      i_id         = 'I'
      i_key        = '000'
      i_text       = 'Object Type'  "#EC NOTEXT
      i_length     = '21',
      ...
      i_id         = 'I'
      i_key        = '002'
      i_text       = 'Total Records Processed'  "#EC NOTEXT
      i_length     = '46',

      i_id         = 'I'
      i_key        = '003'
      i_text       = 'Total Error Count'  "#EC NOTEXT
      i_length     = '27',

      i_id         = 'I'
      i_key        = '004'
      i_text       = 'Business Checks'  "#EC NOTEXT
      i_length     = '25',

      i_id         = 'I'
      i_key        = '005'
      i_text       = 'Action Required'  "#EC NOTEXT
      i_length     = '25',

      i_id         = 'I'
      i_key        = '006'
      i_text       = 'Do you want to delete this record?'  "#EC NOTEXT
      i_length     = '68',
      ...
      i_id         = 'I'
      i_key        = '009'
      i_text       = 'Customer'  "#EC NOTEXT
      i_length     = '18',

      i_id         = 'I'
      i_key        = '010'
      i_text       = 'Vendor'  "#EC NOTEXT
      i_length     = '16',

      i_id         = 'I'
      i_key        = '011'
      i_text       = 'Refresh'  "#EC NOTEXT
      i_length     = '17',

      i_id         = 'I'
      i_key        = '012'
      i_text       = 'Delete'  "#EC NOTEXT
      i_length     = '16',

      i_id         = 'I'
      i_key        = '013'
      i_text       = 'Sr. Num'  "#EC NOTEXT
      i_length     = '17',

      i_id         = 'I'
      i_key        = '014'
      i_text       = 'Serial Number'  "#EC NOTEXT
      i_length     = '23',

      i_id         = 'I'
      i_key        = '015'
      i_text       = 'Object ID'  "#EC NOTEXT
      i_length     = '19',

      i_id         = 'I'
      i_key        = '016'
      i_text       = 'Business Check'  "#EC NOTEXT
      i_length     = '25',
      ...
      i_id         = 'I'
      i_key        = '018'
      i_text       = 'Technical Field Name'  "#EC NOTEXT
      i_length     = '40',

      i_id         = 'I'
      i_key        = '019'
      i_text       = 'Checks'  "#EC NOTEXT
      i_length     = '17',

      i_id         = 'I'
      i_key        = '020'
      i_text       = 'Completed'  "#EC NOTEXT
      i_length     = '19',

      i_id         = 'I'
      i_key        = '021'
      i_text       = 'In Progress'  "#EC NOTEXT
      i_length     = '20',

      i_id         = 'I'
      i_key        = '022'
      i_text       = 'Terminated'  "#EC NOTEXT
      i_length     = '28',

      i_id         = 'I'
      i_key        = '023'
      i_text       = 'Error Description'  "#EC NOTEXT
      i_length     = '27',

      i_id         = 'I'
      i_key        = '024'
      i_text       = 'Field Value'  "#EC NOTEXT
      i_length     = '21'.

ENDFORM.

FORM update_23. " REPT CVI_MIGRATION_PRECHK ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_program_text    "#EC *
    EXPORTING
      i_progname   = 'CVI_MIGRATION_PRECHK'
      i_devclass   = 'CVI_MAPPING'
      i_masterlang = 'E'
      i_langu      = 'E':
      i_id         = 'R'
      i_key        = ''
      i_text       = 'Prevalidation: Master Data Consistency Check'  "#EC NOTEXT
      i_length     = '45',
      ...
      i_id         = 'S'
      i_key        = 'S_CAC_GP'
      i_text       = 'Account Group'  "#EC NOTEXT
      i_length     = '21',

      i_id         = 'S'
      i_key        = 'S_CUST'
      i_text       = 'Customer Number'  "#EC NOTEXT
      i_length     = '23',
      ...
      i_id         = 'S'
      i_key        = 'S_VEND'
      i_text       = 'Vendor Number'  "#EC NOTEXT
      i_length     = '21'.

ENDFORM.

FORM update_24. " CUAD CVI_MIGRATION_PRECHK ( EN , source: E18 )
 DATA:  ls_adm TYPE rsmpe_adm,
  ...
  ls_adm-mencode                 = '000005'.
  ls_adm-pfkcode                 = '000003'.
  CLEAR ls_men.
  ls_men-code                    = '000001'.
  ls_men-no                      = '01'.
  ls_men-ref_type                = 'F'.
  ls_men-ref_code                = 'BACK'.
  ls_men-ref_no                  = '001'.
  ...
  ls_sta-butcode                 = '0001'.
  ls_sta-int_note                = 'CVI Migration Precheck Detail Page GUI Status'. "#EC NOTEXT "#EC TRANSLANG
  ...
  ls_sta-butcode                 = '0001'.
  ls_sta-int_note                = 'Status for SAPSIMPLE_TREE_CONTROL_DEMO'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_sta TO lt_sta.
  ...
  ls_fun-textno                  = '001'.
  ls_fun-text_type               = 'S'.
  ls_fun-fun_text                = 'Enter'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_fun TO lt_fun.
  ...
  ls_fun-icon_id                 = '@15@'.
  ls_fun-fun_text                = 'Execute checks'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_fun TO lt_fun.
  ...
  ls_mtx-code                    = '000001'.
  ls_mtx-text_type               = 'S'.
  ls_mtx-text                    = 'Report'. "#EC NOTEXT "#EC TRANSLANG
  ls_mtx-path                    = 'R'.
  ls_mtx-int_note                = 'Standard Supplement'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_mtx TO lt_mtx.

  CLEAR ls_mtx.
  ls_mtx-code                    = '000002'.
  ls_mtx-text_type               = 'S'.
  ls_mtx-text                    = 'Edit'. "#EC NOTEXT "#EC TRANSLANG
  ls_mtx-path                    = 'E'.
  ls_mtx-int_note                = 'Standard Supplement'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_mtx TO lt_mtx.
  ...
  ls_mtx-code                    = '000003'.
  ls_mtx-text_type               = 'S'.
  ls_mtx-text                    = 'Goto'. "#EC NOTEXT "#EC TRANSLANG
  ls_mtx-path                    = 'G'.
  ls_mtx-int_note                = 'Standard Supplement'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_mtx TO lt_mtx.
  ...
  ls_doc-modal                   = 'D'.
  ls_doc-int_note                = 'PF Status'. "#EC NOTEXT "#EC TRANSLANG
  ...
  ls_doc-modal                   = 'D'.
  ls_doc-int_note                = 'CVI Migration Precheck Detail Page GUI Status'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_doc TO lt_doc.
  ...
  CLEAR ls_tit.
  ls_tit-code                    = 'DETAIL'.
  ls_tit-text                    = 'Master Data Consistency Check - Results'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_tit TO lt_tit.

  CLEAR ls_tit.
  ls_tit-code                    = 'INITIAL'.
  ls_tit-text                    = 'Prevalidation: Master Data Consistency Check'. "#EC NOTEXT "#EC TRANSLANG
  APPEND ls_tit TO lt_tit.
...
FORM update_25. " DOCU RECVI_MIGRATION_PRECHK ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_other_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_docname   = 'RECVI_MIGRATION_PRECHK'
      i_langu     = 'E'
      i_line      =:'U1 &PURPOSE&',                                                                                "#EC NOTEXT
                    'AS This report performs the necessary business checks for customer and',                      "#EC NOTEXT
                    '   vendor master data if you are intending to migrate to SAP S/4HANA. This',                  "#EC NOTEXT
                    '   report gives an upfront overview of master data consistency that is',                      "#EC NOTEXT
                    '   required for conversion to business partner master data. You can run',                     "#EC NOTEXT
                    '   this report to check the following business scenarios:',                                   "#EC NOTEXT
                    'B1 Tax Code',                                                                                 "#EC NOTEXT
                    'B1 Postal Code',                                                                              "#EC NOTEXT
                    'B1 Email',                                                                                    "#EC NOTEXT
                    'B1 Bank Data',                                                                                "#EC NOTEXT
                    'B1 Tax Jurisdiction',                                                                         "#EC NOTEXT
                    'B1 Industry',                                                                                 "#EC NOTEXT
                    'B1 Transportation Zones',                                                                     "#EC NOTEXT
                    'B1 Address',                                                                                  "#EC NOTEXT
                    'B1 Number Range',                                                                             "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'AS You can also add additional checks using the BAdI CVI_CUSTOM_PRECHK',                      "#EC NOTEXT
                    '   (transaction SE18) with custom implementation.',                                           "#EC NOTEXT
                    'U1 &INTEGRATION&',                                                                            "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &PREREQUISITES&',                                                                          "#EC NOTEXT
                    'AS The <LS>Run ID </>is generated based on a customizable number range,',                     "#EC NOTEXT
                    '   which is part of standard content delivery. Note that this content is',                    "#EC NOTEXT
                    '   available in client 000. You should ensure that this content is',                          "#EC NOTEXT
                    '   available in the required clients via client copy.',                                       "#EC NOTEXT
                    'U1 &FEATURES&',                                                                               "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U2 &SELECTION&',                                                                              "#EC NOTEXT
                    'AS Select the parameters for the checks that report need to perform. Then,',                  "#EC NOTEXT
                    '   execute the report.',                                                                      "#EC NOTEXT
                    'B1 <LS>General Selection </>        – You can choose the master data type;',                  "#EC NOTEXT
                    '   Customer or Vendor. You can also enter a customer or vendor number and',                   "#EC NOTEXT
                    '   specify its account group. Note that account group can undergo changes',                   "#EC NOTEXT
                    '   for a given customer or vendor, which may result inconsistent output.',                    "#EC NOTEXT
                    'B1 <LS>Business Checks </>        – Choose the relevant checks that you',                     "#EC NOTEXT
                    '   want to carry out during migration precheck. You can also choose all',                     "#EC NOTEXT
                    '   checks using <LS>Select All </>check box. By default, all the scenarios',                  "#EC NOTEXT
                    '   are selected.',                                                                            "#EC NOTEXT
                    'B1 <LS>Technical Details</>',                                                                 "#EC NOTEXT
                    'B2 <LS>Server Group </>        – You can choose the predefined server group',                 "#EC NOTEXT
                    '   to be used for parallel processing, from the value help. This is a',                       "#EC NOTEXT
                    '   mandatory field.',                                                                         "#EC NOTEXT
                    'B2 <LS>Description </>        – Enter a description for the run. This is a',                  "#EC NOTEXT
                    '   mandatory field.',                                                                         "#EC NOTEXT
                    'B2 <LS>Background </>        – Choose to run the report in the background.',                  "#EC NOTEXT
                    '   The background processing is selected by default.',                                        "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U2 &STANDARD_VARIANTS&',                                                                      "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U2 &OUTPUT&',                                                                                 "#EC NOTEXT
                    'AS The report generates an output containing the status of each check that',                  "#EC NOTEXT
                    '   it performs. The <LS>Run History </>section displays the details of the',                  "#EC NOTEXT
                    '   run with a link to the output <LS>Master Data Consistency Check -',                        "#EC NOTEXT
                    '   Results</>. For foreground mode, the output screen automatically appears',                 "#EC NOTEXT
                    '   after every successful execution.',                                                        "#EC NOTEXT
                    'U2 Run History',                                                                              "#EC NOTEXT
                    'B1 <LS>Status</>',                                                                            "#EC NOTEXT
                    'B2 If the status is green, this indicates that check is completed.',                          "#EC NOTEXT
                    'B2 If the status is yellow, the run is still in progress.',                                   "#EC NOTEXT
                    'B2 If the status is red, the run is terminated. Click <LS>Run ID </>to view',                 "#EC NOTEXT
                    '   the error details.',                                                                       "#EC NOTEXT
                    'B1 <LS>Checks </>- Double click to view the business checks that are',                        "#EC NOTEXT
                    '   considered for the corresponding run.',                                                    "#EC NOTEXT
                    'B1 <LS>Run ID </>- Indicates the unique identifier that is created for each',                 "#EC NOTEXT
                    '   run. Click <LS>Run ID </>to view the detailed output <LS>Master Data',                     "#EC NOTEXT
                    '   Consistency Check - Result</> of the corresponding run.',                                  "#EC NOTEXT
                    'B1 <LS>Description </>- Displays the description for each run that you have',                 "#EC NOTEXT
                    '   previously entered.',                                                                      "#EC NOTEXT
                    'B1 You can use <LS>Refresh </>button to view the latest status of each run.',                 "#EC NOTEXT
                    'B1 You can also select and <LS>Delete </>the completed logs.',                                "#EC NOTEXT
                    'U2 Master Data Consistency Check - Result',                                                   "#EC NOTEXT
                    'AS You can view a detailed list of errors, based on the business checks',                     "#EC NOTEXT
                    '   selected for the run.',                                                                    "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'U1 &ACTIVITIES&',                                                                             "#EC NOTEXT
                    'N1 Enter the relevant parameters and execute the report. The <LS>Server',                     "#EC NOTEXT
                    '   Group </>and <LS>Description </>are mandatory fields.',                                    "#EC NOTEXT
                    'N1 Double click <LS>Checks </>to view the business checks that are',                          "#EC NOTEXT
                    '   considered for the corresponding run.',                                                    "#EC NOTEXT
                    'N1 Click <LS>Run ID </>to view the detailed output <LS>Master Data',                          "#EC NOTEXT
                    '   Consistency Check - Result</> of the corresponding run.',                                  "#EC NOTEXT
                    'U1 &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    space.
...
FORM update_26. " MSAG CVI_PRECHK ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_message_class     "#EC *
    EXPORTING
      i_devclass   = 'CVI_MAPPING'
      i_msgid      = 'CVI_PRECHK'
      i_text       = 'For Storing Messages Related to Prechecks for BP S4 Move'
      i_masterlang = 'E'
      i_langu      = 'E'.

ENDFORM.

FORM update_27. " MESS CVI_PRECHK000 ( EN , source: E18 )
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_text       = 'Input Incomplete for Tax Jurisdiction determination'.

ENDFORM.

FORM update_28. " MESS CVI_PRECHK001 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Tax Number Check failed'.

ENDFORM.

FORM update_29. " MESS CVI_PRECHK003 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Choose atleast one scenario'.

ENDFORM.

FORM update_30. " MESS CVI_PRECHK004 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Cannot process; exceeds the maximum records (&1) for processing'.

ENDFORM.

FORM update_31. " MESS CVI_PRECHK005 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Invalid Server Group'.

ENDFORM.

FORM update_32. " MESS CVI_PRECHK006 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'No server resources available for parallel processing'.

ENDFORM.

FORM update_33. " MESS CVI_PRECHK007 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Unexpected exception occurred in parallel processing'.

ENDFORM.

FORM update_34. " MESS CVI_PRECHK008 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Unexpected exception occurred'.

ENDFORM.

FORM update_35. " MESS CVI_PRECHK009 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'EU Tax Number &1 for Country &2 is incorrect'.

ENDFORM.

FORM update_36. " MESS CVI_PRECHK010 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Tax Type &1 is not allowed for country &2; check table TFKTAXNUMTYPE'.

ENDFORM.

FORM update_37. " MESS CVI_PRECHK011 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Bank Number &1 or Account Number &2 is incorrect for country &3'.

ENDFORM.

FORM update_38. " MESS CVI_PRECHK012 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Bank number &1 must have length &2'.

ENDFORM.

FORM update_39. " MESS CVI_PRECHK013 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Bank Account Number is missing for country &1'.

ENDFORM.

FORM update_40. " MESS CVI_PRECHK014 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = '&1 is not valid for country &2'.

ENDFORM.

FORM update_41. " MESS CVI_PRECHK015 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = '&1 is missing for country &2'.

ENDFORM.

FORM update_42. " MESS CVI_PRECHK016 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Bank &1 &2 does not exist'.

ENDFORM.

FORM update_43. " MESS CVI_PRECHK017 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = '&1 is incorrect.'.

ENDFORM.

FORM update_44. " MESS CVI_PRECHK018 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Length of &1 must not exceed 35 characters.'.

ENDFORM.

FORM update_45. " MESS CVI_PRECHK019 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Form title &1 not allowed for organizations; See SAP Note 1767277.'.

ENDFORM.

FORM update_46. " MESS CVI_PRECHK021 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Server Group is mandatory.'.

ENDFORM.

FORM update_47. " MESS CVI_PRECHK022 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Description is mandatory.'.

ENDFORM.

FORM update_48. " MESS CVI_PRECHK023 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Creation period is mandatory.'.

ENDFORM.

FORM update_49. " MESS CVI_PRECHK024 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Customer number &1 not in range &2 and &3.'.

ENDFORM.

FORM update_50. " MESS CVI_PRECHK025 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Vendor number &1 not in range &2 and &3.'.

ENDFORM.

FORM update_51. " MESS CVI_PRECHK026 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Tax Jurisdiction Code &1 is incorrect for country &2.'.

ENDFORM.

FORM update_52. " MESS CVI_PRECHK027 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Cannot check Tax Jurisdiction Code &1 ; RFC_DEST not found'.

ENDFORM.

FORM update_53. " MESS CVI_PRECHK028 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Transport Zone is mandatory.'.

ENDFORM.

FORM update_54. " MESS CVI_PRECHK029 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Transport Zone &1 is not defined for country &2.'.

ENDFORM.

FORM update_55. " MESS CVI_PRECHK030 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Email Address &1 is incorrect.'.

ENDFORM.

FORM update_56. " MESS CVI_PRECHK031 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Industry mapping for &1 is missing in customizing (Table TP038M2).'.

ENDFORM.

FORM update_57. " MESS CVI_PRECHK032 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Tax Code &1 &2 is incorrect for country &3.'.

ENDFORM.

FORM update_58. " MESS CVI_PRECHK033 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'The selection parameters resulted in 0 records to be checked.'.

ENDFORM.

FORM update_59. " MESS CVI_PRECHK034 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Background processing started.'.

ENDFORM.

FORM update_60. " MESS CVI_PRECHK035 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Run is in progress.'.

ENDFORM.

FORM update_61. " MESS CVI_PRECHK036 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Cannot delete; Select at least one row.'.

ENDFORM.

FORM update_62. " MESS CVI_PRECHK037 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Cannot delete the run(s) that is still in progress.'.

ENDFORM.

FORM update_63. " MESS CVI_PRECHK038 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_64. " MESS CVI_PRECHK039 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'No record found'.

ENDFORM.

FORM update_65. " MESS CVI_PRECHK040 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Cannot process; maintain interval &1 for number range &2'.

ENDFORM.

FORM update_66. " MESS CVI_PRECHK041 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Error while generating Run ID'.

ENDFORM.

FORM update_67. " MSAG CVI_PRECHK_MSG ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_message_class     "#EC *
    EXPORTING
      i_devclass   = 'CVI_MAPPING'
      i_msgid      = 'CVI_PRECHK_MSG'
      i_text       = 'For Storing Messages Related to Prechecks for BP S4 Move'
      i_masterlang = 'E'
      i_langu      = 'E'.
...
FORM update_68. " MESS CVI_PRECHK_MSG000 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_69. " MESS CVI_PRECHK_MSG001 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_70. " MESS CVI_PRECHK_MSG003 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_71. " MESS CVI_PRECHK_MSG004 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_72. " MESS CVI_PRECHK_MSG005 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Enter a valid Server Group'.

ENDFORM.

FORM update_73. " MESS CVI_PRECHK_MSG006 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_74. " MESS CVI_PRECHK_MSG007 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_75. " MESS CVI_PRECHK_MSG008 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_76. " MESS CVI_PRECHK_MSG009 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_77. " MESS CVI_PRECHK_MSG010 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Tax Type &1 is not allowed; check table TFKTAXNUMTYPE'.

ENDFORM.

FORM update_78. " MESS CVI_PRECHK_MSG011 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Bank Number &1 or Account Number &2 is incorrect'.

ENDFORM.

FORM update_79. " MESS CVI_PRECHK_MSG012 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_80. " MESS CVI_PRECHK_MSG013 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Bank Account Number is missing'.

ENDFORM.

FORM update_81. " MESS CVI_PRECHK_MSG014 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_82. " MESS CVI_PRECHK_MSG015 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_83. " MESS CVI_PRECHK_MSG016 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = '& is not valid for country &.'.

ENDFORM.

FORM update_84. " MESS CVI_PRECHK_MSG017 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_85. " MESS CVI_PRECHK_MSG018 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_86. " MESS CVI_PRECHK_MSG019 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_87. " MESS CVI_PRECHK_MSG020 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Successful run'.

ENDFORM.

FORM update_88. " MESS CVI_PRECHK_MSG021 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_89. " MESS CVI_PRECHK_MSG022 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_90. " MESS CVI_PRECHK_MSG023 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_91. " MESS CVI_PRECHK_MSG024 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_92. " MESS CVI_PRECHK_MSG025 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_93. " MESS CVI_PRECHK_MSG026 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Tax Jurisdiction Code &1 is incorrect.'.

ENDFORM.

FORM update_94. " MESS CVI_PRECHK_MSG027 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_95. " MESS CVI_PRECHK_MSG028 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_96. " MESS CVI_PRECHK_MSG029 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_97. " MESS CVI_PRECHK_MSG030 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_98. " MESS CVI_PRECHK_MSG031 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Industry mapping for &1 is missing in table TP038M2.'.

ENDFORM.

FORM update_99. " MESS CVI_PRECHK_MSG032 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Tax Code &1 &2 is incorrect.'.

ENDFORM.

FORM update_100. " MESS CVI_PRECHK_MSG033 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_101. " MESS CVI_PRECHK_MSG034 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_102. " MESS CVI_PRECHK_MSG035 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Run is in process or error-incomplete'.

ENDFORM.

FORM update_103. " MESS CVI_PRECHK_MSG036 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_104. " MESS CVI_PRECHK_MSG037 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
  ...
  CALL METHOD lcl_wb=>create_message     "#EC *
    EXPORTING
      ...
      i_langu      = 'E'
      i_selfdef    = '3'"deletes longtext (DOCU) if neccessary
      i_text       = 'Cannot delete the run(s) that is still in process.'.

ENDFORM.

FORM update_105. " MESS CVI_PRECHK_MSG038 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_106. " MESS CVI_PRECHK_MSG039 ( EN , source: E18 )
  DATA lt_activity TYPE tt_activity.
...
FORM update_107. " TRAN S_EBJ_98001058 ( EN , source: E18 )

  CALL METHOD lcl_wb=>create_logical_object "#EC *
    EXPORTING
      i_object     = 'TRAN'
      i_obj_name   = 'S_EBJ_98001058'
      i_devclass   = 'CVI_MAPPING'
      i_langu      = 'E'
      i_masterlang = 'E'
    EXCEPTIONS
  ...
  CLEAR ls_tstc.
  ls_tstc-tcode     = 'S_EBJ_98001058'.                                                             "#EC NOTEXT
  ls_tstc-pgmna     = 'SAPLS_CUS_IMG_ACTIVITY'.                                                     "#EC NOTEXT
  ls_tstc-dypno     = '0200'.                                                                       "#EC NOTEXT
  ...
  CLEAR ls_tstcc.
  ls_tstcc-tcode         = 'S_EBJ_98001058'.                                                        "#EC NOTEXT
...
FORM update_108. " TRAN CVI_PRECHK ( EN , source: E18 )
  ...
  CLEAR ls_tstc.
  ls_tstc-tcode     = 'CVI_PRECHK'.                                                                 "#EC NOTEXT
  ls_tstc-pgmna     = 'CVI_MIGRATION_PRECHK'.                                                       "#EC NOTEXT
  ls_tstc-dypno     = '1000'.                                                                       "#EC NOTEXT
  APPEND ls_tstc TO lt_tstc.
  ...
  ls_tstct-tcode     = 'CVI_PRECHK'.                                                                "#EC NOTEXT
  ls_tstct-ttext     = 'CVI Masterdata Consistency Check'.                                          "#EC NOTEXT
  ...
  CLEAR ls_tstcc.
  ls_tstcc-tcode         = 'CVI_PRECHK'.                                                            "#EC NOTEXT
  ls_tstcc-s_webgui      = '1'.                                                                     "#EC NOTEXT
  ls_tstcc-s_win32       = 'X'.                                                                     "#EC NOTEXT
  ls_tstcc-s_platin      = 'X'.                                                                     "#EC NOTEXT
  APPEND ls_tstcc TO lt_tstcc.
...
FORM update_109_0002. " TOBJ CVI_PRECHK_SUP ( EN E18 )
  DATA: ls_rpy_dyhead TYPE rpy_dyhead, lt_rpy_dyhead TYPE TABLE OF rpy_dyhead.

  CLEAR ls_rpy_dyhead.
  ls_rpy_dyhead-program        = 'SAPLFG_PRECHK_SUP'.                                               "#EC NOTEXT
  ls_rpy_dyhead-screen         = '0002'.                                                            "#EC NOTEXT
  ls_rpy_dyhead-language       = 'E'.                                                               "#EC NOTEXT
  ls_rpy_dyhead-descript       = 'View maint: Overview screen CVI_PRECHK_SUP'.                      "#EC NOTEXT
  ls_rpy_dyhead-type           = 'N'.                                                               "#EC NOTEXT
  ls_rpy_dyhead-nextscreen     = '0002'.                                                            "#EC NOTEXT
  ls_rpy_dyhead-lines          = '061'.                                                             "#EC NOTEXT
  ls_rpy_dyhead-columns        = '083'.                                                             "#EC NOTEXT
  APPEND ls_rpy_dyhead TO lt_rpy_dyhead.

  DATA: ls_rpy_dycatt TYPE rpy_dycatt, lt_rpy_dycatt TYPE TABLE OF rpy_dycatt.
  ...
  DATA: ls_rpy_dyfatc TYPE rpy_dyfatc, lt_rpy_dyfatc TYPE TABLE OF rpy_dyfatc.
  ...
  ls_rpy_dyfatc-cont_name        = 'TCTRL_CVI_PRECHK_SUP'.                                          "#EC NOTEXT
  ls_rpy_dyfatc-type             = 'TEMPLATE'.                                                      "#EC NOTEXT
  ...
  ls_rpy_dyfatc-name             = '*CVI_PRECHK_SUP-CHECK_TYPE'.                                    "#EC NOTEXT
  ls_rpy_dyfatc-text             = 'Checks'.                                                        "#EC NOTEXT
  ls_rpy_dyfatc-line             = '001'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-column           = '001'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-length           = '006'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-vislength        = '030'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-height           = '001'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-format           = 'CHAR'.                                                          "#EC NOTEXT
  ...
  ls_rpy_dyfatc-cont_name        = 'TCTRL_CVI_PRECHK_SUP'.                                          "#EC NOTEXT
  ls_rpy_dyfatc-type             = 'TEXT'.                                                          "#EC NOTEXT
  ls_rpy_dyfatc-name             = '*CVI_PRECHK_SUP-CHECKED'.                                       "#EC NOTEXT
  ls_rpy_dyfatc-text             = 'Activate'.                                                      "#EC NOTEXT
  ls_rpy_dyfatc-line             = '001'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-column           = '002'.                                                           "#EC NOTEXT
  ls_rpy_dyfatc-length           = '008'.                                                           "#EC NOTEXT
  ...
  DATA: ls_rpy_dypara TYPE rpy_dypara, lt_rpy_dypara TYPE TABLE OF rpy_dypara.
    ...
    APPEND '  TCTRL_CVI_PRECHK_SUP CURSOR NEXTLINE.'                                      TO lt_d022s. "#EC NOTEXT
    ...
    APPEND '    FIELD CVI_PRECHK_SUP-CHECK_TYPE .'                                        TO lt_d022s. "#EC NOTEXT
    APPEND '    FIELD CVI_PRECHK_SUP-CHECKED .'                                           TO lt_d022s. "#EC NOTEXT
...
FORM update_109. " TOBJ CVI_PRECHK_SUPS ( EN , source: E18 )
  IF lcl_wb=>generate_maint_view(  "#EC *
      i_devclass     = 'CVI_MAPPING'
      i_objectname   = 'CVI_PRECHK_SUP'
      i_objecttype   = 'S'
      i_masterlang   = 'E'
      i_auth_group   = '&NC&'
      i_func_group   = 'FG_PRECHK_SUP'
      i_dynp_list    = '0002') EQ 'I'. "Insert
     PERFORM update_109_0002.
  ENDIF.
  CALL METHOD lcl_wb=>create_logical_object "#EC *
    EXPORTING
      i_object   = 'TOBJ'
      i_obj_name = 'CVI_PRECHK_SUPS'
      i_devclass = 'CVI_MAPPING'
      i_langu    = 'E'
    EXCEPTIONS
      OTHERS     = 1.
  ...
  DATA: ls_objh TYPE objh, lt_objh TYPE TABLE OF objh.

  CLEAR ls_objh.
  ls_objh-objectname     = 'CVI_PRECHK_SUP'.                                                        "#EC NOTEXT
  ls_objh-objecttype     = 'S'.                                                                     "#EC NOTEXT
  ls_objh-clidep         = 'X'.                                                                     "#EC NOTEXT
  ls_objh-objcateg       = 'CUST'.                                                                  "#EC NOTEXT
  ls_objh-objtransp      = '2'.                                                                     "#EC NOTEXT
  ls_objh-importable     = '3'.                                                                     "#EC NOTEXT
  APPEND ls_objh TO lt_objh.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'TOBJ'
      i_tabname  = 'OBJH'
      it_entries = lt_objh.

  DATA: ls_objs TYPE objs, lt_objs TYPE TABLE OF objs.

  CLEAR ls_objs.
  ls_objs-objectname     = 'CVI_PRECHK_SUP'.                                                        "#EC NOTEXT
  ls_objs-objecttype     = 'S'.                                                                     "#EC NOTEXT
  ls_objs-tabname        = 'CVI_PRECHK_SUP'.                                                        "#EC NOTEXT
  ls_objs-ddic           = 'X'.                                                                     "#EC NOTEXT
  ls_objs-prim_table     = 'X'.                                                                     "#EC NOTEXT
...
FORM update_110. " DSYS SIMGCVI_ CUSTOM_PRECHECK ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_other_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_docname   = 'HYSIMGCVI_ CUSTOM_PRECHECK'
      i_langu     = 'E'
      i_line      =:'UT &USE&',                                                                                    "#EC NOTEXT
                    'AS This Business Add-In (BAdI) is used in the <LS>Master Data',                               "#EC NOTEXT
                    '   Synchronization (AP-MD-BF-SYN) </>component.',                                             "#EC NOTEXT
                    'AS You use this BAdI to add additional checks in the',                                        "#EC NOTEXT
                    '=   <DS:RE.CVI_MIGRATION_PRECHK>Prevalidation: Master Data Consistency',                      "#EC NOTEXT
                    '   Check</> (CVI_MIGRATION_PRECHK) report using custom logic.',                               "#EC NOTEXT
                    'AS This BAdI uses the interface <ZH>IF_CVI_CUSTOM_PRECHK</>.',                                "#EC NOTEXT
                    'AS You can implement the <ZH>EXECUTE_CUSTOM_CHECK</> method from the',                        "#EC NOTEXT
                    '   interface in which you can include all the relevant checks that the',                      "#EC NOTEXT
                    '   system should perform for customer or vendor, as per requirement. This',                   "#EC NOTEXT
                    '   method has the following parameters:',                                                     "#EC NOTEXT
                    'AS <LS>Importing Parameters:</>',                                                             "#EC NOTEXT
                    'B1 <ZH>IT_CUSTOMER </>- Contains the customer number',                                        "#EC NOTEXT
                    'B1 <ZH>IT_VENDOR </>- Contains the vendor number',                                            "#EC NOTEXT
                    'B1 Note that only one of the parameters will have a value during execution,',                 "#EC NOTEXT
                    '   based on customer specifications.',                                                        "#EC NOTEXT
                    'AS <LS>Exporting Parameters</>',                                                              "#EC NOTEXT
                    'B1 <ZH>CT_ERROR </>- Error messages after the checks are performed',                          "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'UT &PRECONDITIONS&',                                                                          "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'UT &STANDARD_SETUP&',                                                                         "#EC NOTEXT
                    'AS For more information about the standard settings (filters, single or',                     "#EC NOTEXT
                    '   multiple uses), see the Enhancement Spot Element Definitions tab in the',                  "#EC NOTEXT
                    '   BAdI Builder (transaction SE18).',                                                         "#EC NOTEXT
                    'UT &ACTIVITY&',                                                                               "#EC NOTEXT
                    '*  The',                                                                                      "#EC NOTEXT
                    '=   <DS:RE.CVI_MIGRATION_PRECHK>Prevalidation: Master Data Consistency',                      "#EC NOTEXT
                    '   Check</> (CVI_MIGRATION_PRECHK) report calls this BAdI after performing',                  "#EC NOTEXT
                    '   the standard checks.',                                                                     "#EC NOTEXT
                    'UT &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    space.
ENDFORM.

FORM update_111. " DSYS SIMGCVI_CUSTOM_PRECHK ( EN , source: E18 )
  CALL METHOD lcl_wb=>create_other_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_docname   = 'HYSIMGCVI_CUSTOM_PRECHK'
      i_langu     = 'E'
      i_line      =:'UT &USE&',                                                                                    "#EC NOTEXT
                    'AS This Business Add-In (BAdI) is used in the <LS>Master Data',                               "#EC NOTEXT
                    '   Synchronization (AP-MD-BF-SYN) </>component.',                                             "#EC NOTEXT
                    'AS You use this BAdI to add checks in the',                                                   "#EC NOTEXT
                    '=   <DS:RE.CVI_MIGRATION_PRECHK>Prevalidation: Master Data Consistency',                      "#EC NOTEXT
                    '   Check</> (CVI_MIGRATION_PRECHK) report in addition to standard cheks.',                    "#EC NOTEXT
                    '*  The',                                                                                      "#EC NOTEXT
                    '=   <DS:RE.CVI_MIGRATION_PRECHK>Prevalidation: Master Data Consistency',                      "#EC NOTEXT
                    '   Check</> (CVI_MIGRATION_PRECHK) report calls this BAdI while performing',                  "#EC NOTEXT
                    '   the standard checks.',                                                                     "#EC NOTEXT
                    'UT &PRECONDITIONS&',                                                                          "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'UT &STANDARD_SETUP&',                                                                         "#EC NOTEXT
                    ...
                    '   BAdI Builder (transaction SE18).',                                                         "#EC NOTEXT
                    'UT &ACTIVITY&',                                                                               "#EC NOTEXT
                    'AS This BAdI definition uses the interface <ZH>IF_CVI_CUSTOM_PRECHK</>. For',                 "#EC NOTEXT
                    '   more information, display the interface in the Class Builder.',                            "#EC NOTEXT
                    'AS You can implement the <ZH>EXECUTE_CUSTOM_CHECK</> method in which you',                    "#EC NOTEXT
                    '   can include all relevant checks that the system should perform for',                       "#EC NOTEXT
                    '   customer or vendor, as per requirement. This method has the following',                    "#EC NOTEXT
                    '   parameters:',                                                                              "#EC NOTEXT
                    'AS <LS>Importing Parameters:</>',                                                             "#EC NOTEXT
                    'B1 <ZH>IT_CUSTOMER </>- Contains the customer number',                                        "#EC NOTEXT
                    'B1 <ZH>IT_VENDOR </>- Contains the vendor number',                                            "#EC NOTEXT
                    'B1 Note that only one of the parameters will have a value during execution,',                 "#EC NOTEXT
                    '   based on your selection.',                                                                 "#EC NOTEXT
                    'AS <LS>Exporting Parameters</>',                                                              "#EC NOTEXT
                    'B1 <ZH>CT_ERROR </>- Error messages after the checks are performed',                          "#EC NOTEXT
                    'UT &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
...
FORM update_112. " DSYS SIMGCVI_PRECHK_SUP ( DE , source: E18 )
  CALL METHOD lcl_wb=>create_other_docu     "#EC *
    EXPORTING
      i_devclass  = 'CVI_MAPPING'
      i_docname   = 'HYSIMGCVI_PRECHK_SUP'
      i_langu     = 'D'
      i_line      =:'UT &USE&',                                                                                    "#EC NOTEXT
                    'AS Mit dieser Customizing-Aktivität können Sie bei der Konvertierung von',                    "#EC NOTEXT
                    '   Debitor/Kreditor zu Geschäftspartner im Rahmen der Migration nach SAP',                    "#EC NOTEXT
                    '   S/4HANA die Prüfung von Feldern unterdrücken, die Sie umgehen möchten.',                   "#EC NOTEXT
                    'UT &PRECONDITIONS&',                                                                          "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'UT &STANDARD_SETUP&',                                                                         "#EC NOTEXT
                    'AS Es steht eine Default-BAdI-Implementierung (CVI_CUSTOM_MAPPER) zur',                       "#EC NOTEXT
                    '   Verfügung, die bei der Konvertierung von Debitor/Kreditor zu',                             "#EC NOTEXT
                    '   Geschäftspartner aufgerufen wird und die Prüfung folgender Felder',                        "#EC NOTEXT
                    '   unterdrückt:',                                                                             "#EC NOTEXT
                    'B1 Postleitzahl',                                                                             "#EC NOTEXT
                    'B1 Steuerstandort',                                                                           "#EC NOTEXT
                    'B1 Land',                                                                                     "#EC NOTEXT
                    'B1 Region',                                                                                   "#EC NOTEXT
                    'B1 E-Mail',                                                                                   "#EC NOTEXT
                    'B1 Bank',                                                                                     "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    'UT &ACTIVITY&',                                                                               "#EC NOTEXT
                    '*  Markieren Sie das Ankreuzfeld <LS>Aktivieren </>für die Felder, deren',                    "#EC NOTEXT
                    '   Prüfung Sie unterdrücken möchten.',                                                        "#EC NOTEXT
                    'UT &EXAMPLE&',                                                                                "#EC NOTEXT
                    'AS',                                                                                          "#EC NOTEXT
                    space.
ENDFORM.

FORM update_113. " CUS0 CVI_PRECHK_SUP ( EN , source: E18 )

  CALL METHOD lcl_wb=>create_logical_object "#EC *
    EXPORTING
      i_object     = 'CUS0'
      i_obj_name   = 'CVI_PRECHK_SUP'
      i_devclass   = 'CVI_MAPPING'
      i_langu      = 'E'
      i_masterlang = 'E'
    EXCEPTIONS
  ...
  DATA: ls_cus_imgach TYPE cus_imgach, lt_cus_imgach TYPE TABLE OF cus_imgach.

  CLEAR ls_cus_imgach.
  ls_cus_imgach-activity       = 'CVI_PRECHK_SUP'.                                                  "#EC NOTEXT
  ls_cus_imgach-docu_id        = 'SIMGCVI_PRECHK_SUP'.                                              "#EC NOTEXT
  ls_cus_imgach-attributes     = 'CVI_PRECHK_SUP'.                                                  "#EC NOTEXT
  ls_cus_imgach-c_activity     = 'CVI_PRECHK_SUP'.                                                  "#EC NOTEXT
  ls_cus_imgach-tcode          = 'S_EBJ_98001058'.                                                  "#EC NOTEXT
  ls_cus_imgach-fuser          = sy-uname.                                                          "#EC NOTEXT
  ls_cus_imgach-fdate          = sy-datum.                                                          "#EC NOTEXT
  ls_cus_imgach-ftime          = sy-uzeit.                                                          "#EC NOTEXT
  ls_cus_imgach-luser          = sy-uname.                                                          "#EC NOTEXT
  ls_cus_imgach-ldate          = sy-datum.                                                          "#EC NOTEXT
  ls_cus_imgach-ltime          = sy-uzeit.                                                          "#EC NOTEXT
  APPEND ls_cus_imgach TO lt_cus_imgach.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'CUS0'
      i_tabname  = 'CUS_IMGACH'
      it_entries = lt_cus_imgach.

  DATA: ls_cus_imgact TYPE cus_imgact, lt_cus_imgact TYPE TABLE OF cus_imgact.

  CLEAR ls_cus_imgact.
  ls_cus_imgact-spras        = 'E'.                                                                 "#EC NOTEXT
  ls_cus_imgact-activity     = 'CVI_PRECHK_SUP'.                                                    "#EC NOTEXT
  ls_cus_imgact-text         = 'Activate Field Check Suppression'.                                  "#EC NOTEXT
  APPEND ls_cus_imgact TO lt_cus_imgact.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'CUS0'
      i_tabname  = 'CUS_IMGACT'
      it_entries = lt_cus_imgact.

ENDFORM.

FORM update_114. " CUS1 CVI_PRECHK_SUP ( EN , source: E18 )
  ...
  DATA: ls_cus_acth TYPE cus_acth, lt_cus_acth TYPE TABLE OF cus_acth.

  CLEAR ls_cus_acth.
  ls_cus_acth-act_id        = 'CVI_PRECHK_SUP'.                                                     "#EC NOTEXT
  ls_cus_acth-act_type      = 'C'.                                                                  "#EC NOTEXT
  ls_cus_acth-fuser         = sy-uname.                                                             "#EC NOTEXT
  ls_cus_acth-luser         = sy-uname.                                                             "#EC NOTEXT
  CONVERT DATE sy-datlo TIME sy-timlo INTO TIME STAMP ls_cus_acth-fdatetime TIME ZONE sy-zonlo.
  CONVERT DATE sy-datlo TIME sy-timlo INTO TIME STAMP ls_cus_acth-ldatetime TIME ZONE sy-zonlo.
  APPEND ls_cus_acth TO lt_cus_acth.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'CUS1'
      i_tabname  = 'CUS_ACTH'
      it_entries = lt_cus_acth.

  DATA: ls_cus_actobj TYPE cus_actobj, lt_cus_actobj TYPE TABLE OF cus_actobj.

  CLEAR ls_cus_actobj.
  ls_cus_actobj-act_id         = 'CVI_PRECHK_SUP'.                                                  "#EC NOTEXT
  ls_cus_actobj-objecttype     = 'S'.                                                               "#EC NOTEXT
  ls_cus_actobj-objectname     = 'CVI_PRECHK_SUP'.                                                  "#EC NOTEXT
  ls_cus_actobj-tcode          = 'SM30'.                                                            "#EC NOTEXT
  ls_cus_actobj-img_pos        = '01'.                                                              "#EC NOTEXT
  APPEND ls_cus_actobj TO lt_cus_actobj.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'CUS1'
      i_tabname  = 'CUS_ACTOBJ'
      it_entries = lt_cus_actobj.

  DATA: ls_cus_actt TYPE cus_actt, lt_cus_actt TYPE TABLE OF cus_actt.

  CLEAR ls_cus_actt.
  ls_cus_actt-spras      = 'E'.                                                                     "#EC NOTEXT
  ls_cus_actt-act_id     = 'CVI_PRECHK_SUP'.                                                        "#EC NOTEXT
  ls_cus_actt-text       = 'Activate Field-Check Suppression'.                                      "#EC NOTEXT
  APPEND ls_cus_actt TO lt_cus_actt.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'CUS1'
      i_tabname  = 'CUS_ACTT'
      it_entries = lt_cus_actt.

ENDFORM.

FORM update_115. " CUS2 CVI_PRECHK_SUP ( EN , source: E18 )
  ...
  DATA: ls_cus_atrh TYPE cus_atrh, lt_cus_atrh TYPE TABLE OF cus_atrh.

  CLEAR ls_cus_atrh.
  ls_cus_atrh-attr_id        = 'CVI_PRECHK_SUP'.                                                    "#EC NOTEXT
  ls_cus_atrh-roadmap_id     = '105'.                                                               "#EC NOTEXT
  ls_cus_atrh-country        = 'A'.                                                                 "#EC NOTEXT
  ls_cus_atrh-activity       = '2'.                                                                 "#EC NOTEXT
  ls_cus_atrh-critical       = '2'.                                                                 "#EC NOTEXT
  ls_cus_atrh-fuser          = sy-uname.                                                            "#EC NOTEXT
  ls_cus_atrh-luser          = sy-uname.                                                            "#EC NOTEXT
  CONVERT DATE sy-datlo TIME sy-timlo INTO TIME STAMP ls_cus_atrh-fdatetime TIME ZONE sy-zonlo.
  CONVERT DATE sy-datlo TIME sy-timlo INTO TIME STAMP ls_cus_atrh-ldatetime TIME ZONE sy-zonlo.
  APPEND ls_cus_atrh TO lt_cus_atrh.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'CUS2'
      i_tabname  = 'CUS_ATRH'
      it_entries = lt_cus_atrh.

  DATA: ls_cus_atrt TYPE cus_atrt, lt_cus_atrt TYPE TABLE OF cus_atrt.

  CLEAR ls_cus_atrt.
  ls_cus_atrt-spras       = 'E'.                                                                    "#EC NOTEXT
  ls_cus_atrt-attr_id     = 'CVI_PRECHK_SUP'.                                                       "#EC NOTEXT
  ls_cus_atrt-text        = 'Activate Field-Check Suppression'.                                     "#EC NOTEXT
  APPEND ls_cus_atrt TO lt_cus_atrt.

  CALL METHOD lcl_wb=>create_table_entries "#EC *
    EXPORTING
      i_logical_object = 'CUS2'
      i_tabname  = 'CUS_ATRT'
      it_entries = lt_cus_atrt.

  DATA: ls_tfm18 TYPE tfm18, lt_tfm18 TYPE TABLE OF tfm18.

  CLEAR ls_tfm18.
  ls_tfm18-dokclass     = 'SIMG'.                                                                   "#EC NOTEXT
  ls_tfm18-dokname      = 'CVI_PRECHK_SUP'.                                                         "#EC NOTEXT
  ls_tfm18-funct        = 'PAB0000028'.                                                             "#EC NOTEXT
  APPEND ls_tfm18 TO lt_tfm18.
...
FORM genview.
  ...
  PERFORM update_109. " TOBJ CVI_PRECHK_SUPS ( EN , source: E18 )
ENDFORM.
...
FORM update.
  ...
  PERFORM update_01. " NROB CVI_PRECHK ( EN , source: E18 )
  PERFORM update_02. " FUGR CVI_PRECHK ( EN , source: E18 )
  PERFORM update_03. " FUGR FG_PRECHK_SUP ( EN , source: E18 )
  PERFORM update_04. " DOCU DECVI_PRECHK_ADDR ( EN , source: E18 )
  PERFORM update_05. " DOCU DECVI_PRECHK_BANK ( EN , source: E18 )
  PERFORM update_06. " DOCU DECVI_PRECHK_BGMODE ( EN , source: E18 )
  PERFORM update_07. " DOCU DECVI_PRECHK_EMAIL ( EN , source: E18 )
  PERFORM update_08. " DOCU DECVI_PRECHK_INDUS ( EN , source: E18 )
  PERFORM update_09. " DOCU DECVI_PRECHK_NUMRANGE ( EN , source: E18 )
  PERFORM update_10. " DOCU DECVI_PRECHK_OBJTYPE ( EN , source: E18 )
  PERFORM update_11. " DOCU DECVI_PRECHK_PCODE ( EN , source: E18 )
  PERFORM update_12. " DOCU DECVI_PRECHK_RUNID ( EN , source: E18 )
  PERFORM update_13. " DOCU DECVI_PRECHK_RUN_DESC ( EN , source: E18 )
  PERFORM update_14. " DOCU DECVI_PRECHK_SCENARIO ( EN , source: E18 )
  PERFORM update_15. " DOCU DECVI_PRECHK_SELECTALL ( EN , source: E18 )
  PERFORM update_16. " DOCU DECVI_PRECHK_STATUS ( EN , source: E18 )
  PERFORM update_17. " DOCU DECVI_PRECHK_SUP_IND ( EN , source: E18 )
  PERFORM update_18. " DOCU DECVI_PRECHK_SUP_TYPE ( EN , source: E18 )
  PERFORM update_19. " DOCU DECVI_PRECHK_TAXCODE ( EN , source: E18 )
  PERFORM update_20. " DOCU DECVI_PRECHK_TAX_J ( EN , source: E18 )
  PERFORM update_21. " DOCU DECVI_PRECHK_TZONE ( EN , source: E18 )
  PERFORM update_22. " REPT CL_CVI_PRECHK_UI==============CP ( EN , source: E18 )
  PERFORM update_23. " REPT CVI_MIGRATION_PRECHK ( EN , source: E18 )
  PERFORM update_24. " CUAD CVI_MIGRATION_PRECHK ( EN , source: E18 )
  PERFORM update_25. " DOCU RECVI_MIGRATION_PRECHK ( EN , source: E18 )
  PERFORM update_26. " MSAG CVI_PRECHK ( EN , source: E18 )
  PERFORM update_27. " MESS CVI_PRECHK000 ( EN , source: E18 )
  PERFORM update_28. " MESS CVI_PRECHK001 ( EN , source: E18 )
  PERFORM update_29. " MESS CVI_PRECHK003 ( EN , source: E18 )
  PERFORM update_30. " MESS CVI_PRECHK004 ( EN , source: E18 )
  PERFORM update_31. " MESS CVI_PRECHK005 ( EN , source: E18 )
  PERFORM update_32. " MESS CVI_PRECHK006 ( EN , source: E18 )
  PERFORM update_33. " MESS CVI_PRECHK007 ( EN , source: E18 )
  PERFORM update_34. " MESS CVI_PRECHK008 ( EN , source: E18 )
  PERFORM update_35. " MESS CVI_PRECHK009 ( EN , source: E18 )
  PERFORM update_36. " MESS CVI_PRECHK010 ( EN , source: E18 )
  PERFORM update_37. " MESS CVI_PRECHK011 ( EN , source: E18 )
  PERFORM update_38. " MESS CVI_PRECHK012 ( EN , source: E18 )
  PERFORM update_39. " MESS CVI_PRECHK013 ( EN , source: E18 )
  PERFORM update_40. " MESS CVI_PRECHK014 ( EN , source: E18 )
  PERFORM update_41. " MESS CVI_PRECHK015 ( EN , source: E18 )
  PERFORM update_42. " MESS CVI_PRECHK016 ( EN , source: E18 )
  PERFORM update_43. " MESS CVI_PRECHK017 ( EN , source: E18 )
  PERFORM update_44. " MESS CVI_PRECHK018 ( EN , source: E18 )
  PERFORM update_45. " MESS CVI_PRECHK019 ( EN , source: E18 )
  PERFORM update_46. " MESS CVI_PRECHK021 ( EN , source: E18 )
  PERFORM update_47. " MESS CVI_PRECHK022 ( EN , source: E18 )
  PERFORM update_48. " MESS CVI_PRECHK023 ( EN , source: E18 )
  PERFORM update_49. " MESS CVI_PRECHK024 ( EN , source: E18 )
  PERFORM update_50. " MESS CVI_PRECHK025 ( EN , source: E18 )
  PERFORM update_51. " MESS CVI_PRECHK026 ( EN , source: E18 )
  PERFORM update_52. " MESS CVI_PRECHK027 ( EN , source: E18 )
  PERFORM update_53. " MESS CVI_PRECHK028 ( EN , source: E18 )
  PERFORM update_54. " MESS CVI_PRECHK029 ( EN , source: E18 )
  PERFORM update_55. " MESS CVI_PRECHK030 ( EN , source: E18 )
  PERFORM update_56. " MESS CVI_PRECHK031 ( EN , source: E18 )
  PERFORM update_57. " MESS CVI_PRECHK032 ( EN , source: E18 )
  PERFORM update_58. " MESS CVI_PRECHK033 ( EN , source: E18 )
  PERFORM update_59. " MESS CVI_PRECHK034 ( EN , source: E18 )
  PERFORM update_60. " MESS CVI_PRECHK035 ( EN , source: E18 )
  PERFORM update_61. " MESS CVI_PRECHK036 ( EN , source: E18 )
  PERFORM update_62. " MESS CVI_PRECHK037 ( EN , source: E18 )
  PERFORM update_63. " MESS CVI_PRECHK038 ( EN , source: E18 )
  PERFORM update_64. " MESS CVI_PRECHK039 ( EN , source: E18 )
  PERFORM update_65. " MESS CVI_PRECHK040 ( EN , source: E18 )
  PERFORM update_66. " MESS CVI_PRECHK041 ( EN , source: E18 )
  PERFORM update_67. " MSAG CVI_PRECHK_MSG ( EN , source: E18 )
  PERFORM update_68. " MESS CVI_PRECHK_MSG000 ( EN , source: E18 )
  PERFORM update_69. " MESS CVI_PRECHK_MSG001 ( EN , source: E18 )
  PERFORM update_70. " MESS CVI_PRECHK_MSG003 ( EN , source: E18 )
  PERFORM update_71. " MESS CVI_PRECHK_MSG004 ( EN , source: E18 )
  PERFORM update_72. " MESS CVI_PRECHK_MSG005 ( EN , source: E18 )
  PERFORM update_73. " MESS CVI_PRECHK_MSG006 ( EN , source: E18 )
  PERFORM update_74. " MESS CVI_PRECHK_MSG007 ( EN , source: E18 )
  PERFORM update_75. " MESS CVI_PRECHK_MSG008 ( EN , source: E18 )
  PERFORM update_76. " MESS CVI_PRECHK_MSG009 ( EN , source: E18 )
  PERFORM update_77. " MESS CVI_PRECHK_MSG010 ( EN , source: E18 )
  PERFORM update_78. " MESS CVI_PRECHK_MSG011 ( EN , source: E18 )
  PERFORM update_79. " MESS CVI_PRECHK_MSG012 ( EN , source: E18 )
  PERFORM update_80. " MESS CVI_PRECHK_MSG013 ( EN , source: E18 )
  PERFORM update_81. " MESS CVI_PRECHK_MSG014 ( EN , source: E18 )
  PERFORM update_82. " MESS CVI_PRECHK_MSG015 ( EN , source: E18 )
  PERFORM update_83. " MESS CVI_PRECHK_MSG016 ( EN , source: E18 )
  PERFORM update_84. " MESS CVI_PRECHK_MSG017 ( EN , source: E18 )
  PERFORM update_85. " MESS CVI_PRECHK_MSG018 ( EN , source: E18 )
  PERFORM update_86. " MESS CVI_PRECHK_MSG019 ( EN , source: E18 )
  PERFORM update_87. " MESS CVI_PRECHK_MSG020 ( EN , source: E18 )
  PERFORM update_88. " MESS CVI_PRECHK_MSG021 ( EN , source: E18 )
  PERFORM update_89. " MESS CVI_PRECHK_MSG022 ( EN , source: E18 )
  PERFORM update_90. " MESS CVI_PRECHK_MSG023 ( EN , source: E18 )
  PERFORM update_91. " MESS CVI_PRECHK_MSG024 ( EN , source: E18 )
  PERFORM update_92. " MESS CVI_PRECHK_MSG025 ( EN , source: E18 )
  PERFORM update_93. " MESS CVI_PRECHK_MSG026 ( EN , source: E18 )
  PERFORM update_94. " MESS CVI_PRECHK_MSG027 ( EN , source: E18 )
  PERFORM update_95. " MESS CVI_PRECHK_MSG028 ( EN , source: E18 )
  PERFORM update_96. " MESS CVI_PRECHK_MSG029 ( EN , source: E18 )
  PERFORM update_97. " MESS CVI_PRECHK_MSG030 ( EN , source: E18 )
  PERFORM update_98. " MESS CVI_PRECHK_MSG031 ( EN , source: E18 )
  PERFORM update_99. " MESS CVI_PRECHK_MSG032 ( EN , source: E18 )
  PERFORM update_100. " MESS CVI_PRECHK_MSG033 ( EN , source: E18 )
  PERFORM update_101. " MESS CVI_PRECHK_MSG034 ( EN , source: E18 )
  PERFORM update_102. " MESS CVI_PRECHK_MSG035 ( EN , source: E18 )
  PERFORM update_103. " MESS CVI_PRECHK_MSG036 ( EN , source: E18 )
  PERFORM update_104. " MESS CVI_PRECHK_MSG037 ( EN , source: E18 )
  PERFORM update_105. " MESS CVI_PRECHK_MSG038 ( EN , source: E18 )
  PERFORM update_106. " MESS CVI_PRECHK_MSG039 ( EN , source: E18 )
  PERFORM update_107. " TRAN S_EBJ_98001058 ( EN , source: E18 )
  PERFORM update_108. " TRAN CVI_PRECHK ( EN , source: E18 )
  PERFORM update_109. " TOBJ CVI_PRECHK_SUPS ( EN , source: E18 )
  PERFORM update_110. " DSYS SIMGCVI_ CUSTOM_PRECHECK ( EN , source: E18 )
  PERFORM update_111. " DSYS SIMGCVI_CUSTOM_PRECHK ( EN , source: E18 )
  PERFORM update_112. " DSYS SIMGCVI_PRECHK_SUP ( DE , source: E18 )
  PERFORM update_113. " CUS0 CVI_PRECHK_SUP ( EN , source: E18 )
  PERFORM update_114. " CUS1 CVI_PRECHK_SUP ( EN , source: E18 )
  PERFORM update_115. " CUS2 CVI_PRECHK_SUP ( EN , source: E18 )
