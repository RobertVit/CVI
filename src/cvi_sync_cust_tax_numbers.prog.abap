...
* -------------------------------------------------------
  application_type    type bank_dte_pp_paapplcatg value 'CVI_SCUTAX',"#EC NOTEXT
* -------------------------------------------------------

  direction_constant  type char6 value 'CVISCT'. " CVI Sync Customer Tax data  "#EC NOTEXT
...
parameters:
  ...
  p_bpcust type c radiobutton group dir modif id md1,
  p_custbp type c radiobutton group dir modif id md1.
...
* _______________________________________________________
  ...
  if p_bpcust is not initial.
    ls_appl_params-tax_mapping_direction = '1'.
  else.
    ls_appl_params-tax_mapping_direction = '2'.
  endif.
...
*   save the direction to BPCONSTANTS table
    ...
    ls_bpconst-value(1)   = p_bpcust .
    ls_bpconst-value+1(1) = p_custbp .
...
*     format: 123 where character 1-p_bpcust 2-p_custbp 3-p_soft_d
    p_bpcust = ls_bpconst-value(1).
    p_custbp = ls_bpconst-value+1(1).
    p_soft_d = ls_bpconst-value+2(1).
    assert p_bpcust <> p_custbp.
    loop at screen.
